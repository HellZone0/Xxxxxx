...
local RockHubConfig = Window.ConfigManager:CreateConfig("HellZone")
...
local BaseFolder = "WindUI/" .. (Window.Folder or "HellZone") .. "/config/"
...
do
    ...
    if PromptController and PromptController.FirePrompt and Promise then
        ...
        warn("[HellZone] Gagal memuat PromptController/Promise untuk Auto Accept Trade.")
    end
end
        ...
        if data then
            ...
            if targetPlayer.Character then
                for _, v in ipairs(targetPlayer.Character:GetChildren()) do
                    if v.Name == "HellZoneESP" and v:IsA("BillboardGui") then pcall(function() v:Destroy() end) end
                end
            end
        end
    end
    ...
    task.spawn(function()
        ...
        if S1 and FishingController then
            ...
            FishingController.SendFishingRequestToServer = function(...)
                if _G.RockHub_BlatantActive then return false, "Blocked by HellZone" end
                return Old_Cast(...)
            end
        end
    end)
...
do
    ...
    local function CreateKaitunUI()
        local old = game.CoreGui:FindFirstChild("HellZoneKaitunStats")
        if old then old:Destroy() end
        local sg = Instance.new("ScreenGui")
        sg.Name = "HellZoneKaitunStats"
        ...
        txt("KAITUN HELLZONE (PREMIUM)", 0.2, Color3.fromRGB(255,0,255), 35)
        ...
        Callback = function(state)
            ...
            if state then
                ...
                if not _G.BlackScreenGUI then
                    ...
                    _G.BlackScreenGUI.Name = "HellZone_BlackBackground"
        ...
        Callback = function(state)
            ...
            if state then
                ...
                for _, gui in ipairs(PlayerGui:GetChildren()) do
                    if gui:IsA("ScreenGui") and gui.Name ~= "WindUI" and gui.Name ~= "CustomFloatingIcon_HellZone" then
...
do
    ...
    local WEBHOOK_URL = ""
    local WEBHOOK_USERNAME = "HellZone Notify" 
    ...
    local function onFishObtained(itemId, metadata, fullData)
        local success, results = pcall(function()
            ...
            if isWebhookEnabled and WEBHOOK_URL ~= "" and isUserFilterMatch then
                ...
                local embed = {
                    ...
                    footer = {
                        text = string.format("HellZone Webhook • Total Caught: %s • %s", caughtDisplay, os.date("%Y-%m-%d %H:%M:%S"))
                    }
                }
            ...
            if isGlobalTarget and GLOBAL_WEBHOOK_URL ~= "" then 
                ...
                local globalEmbed = {
                    ...
                    thumbnail = { url = imageUrl },
                    footer = {
                        text = string.format("HellZone Community| Player: %s | %s", censoredPlayerName, os.date("%Y-%m-%d %H:%M:%S"))
                    }
                }
        ...
        if not success then
            warn("[HellZone Webhook] Error processing fish data:", results)
        end
    end
...
do
    ...
    local SelectedConfigName = "HellZone" -- Default
    local BaseFolder = "WindUI/" .. (Window.Folder or "HellZone") .. "/config/"
    ...
    ConfigSection:Button({
        ...
        Callback = function()
            ...
            if SelectedConfigName ~= "HellZone" then
                local success, err = pcall(function()
                    local mainContent = readfile(BaseFolder .. "HellZone.json")
    ...
    ConfigSection:Button({
        ...
        Callback = function()
            if SelectedConfigName == "" or SelectedConfigName == "HellZone" then 
            ...
            if isfile(path) then
                ...
                RefreshConfigList(ConfigDropdown)
                ConfigNameInput:Set("HellZone")
                SelectedConfigName = "HellZone"
            else
...
do
    ...
    about:Section({
        Title = "Join Discord Server HellZone",
        TextSize = 20,
    })
...
local function CreateFloatingIcon()
    local existingGui = PlayerGui:FindFirstChild("CustomFloatingIcon_HellZone")
    if existingGui then existingGui:Destroy() end

    local FloatingIconGui = Instance.new("ScreenGui")
    FloatingIconGui.Name = "CustomFloatingIcon_HellZone"
...
WindUI:Notify({ Title = "HellZone Was Loaded", Content = "Press [F] to open/close the menu", Duration = 5, Icon = "info" })
-- [[ AUTO LOAD & SAVE LOOP ]]
task.spawn(function()
    task.wait(2) -- Tunggu UI load sempurna
    
    -- Ganti Load biasa dengan Smart Load
    -- Default load "HellZone", atau nama config terakhir user kalau kamu simpan
    SmartLoadConfig("HellZone") 
